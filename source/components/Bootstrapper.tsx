...
export const Bootstrapper: React.FC<BootstrapperProps> = ({onReady}) => {
	...
	// Spinner animation
	useEffect(() => {
		if (status === 'installed' || status === 'success' || status === 'skipped')
			return;
	...
	useEffect(() => {
		...
		const run = async () => {
			...
			setMessage(
				'Configurando seguridad inicial. Por favor, acepta los permisos de Administrador...',
			);
			...
			{status === 'error' ? (
				<Box flexDirection="column">
					...
					<Box marginTop={1}>
						<Text dimColor>
							{' '}
							Cierra y vuelve a abrir el programa para reintentar.
						</Text>
					</Box>
				</Box>
			) : (

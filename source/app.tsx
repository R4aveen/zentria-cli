...
import {Clock} from './components/common/Clock.js';
	...
	return (
		...
		>
				...
				<Box>
					...
					<Box>
						<Text
							...
							backgroundColor={
								mode === 'online'
									? theme.modeBadgeOnline
									: theme.modeBadgeOffline
							}
						>
				...
				{isWide && (
					<Box>
						<Text color={theme.textDim}>
							⌗ SYS.BR-{AuthService.getBranchId()} │ 【┘】{' '}
							<Clock format="full" />
						</Text>
					</Box>
				)}
			...
			<Box
				...
				flexDirection="column"
				justifyContent={
					view === 'menu' || view === 'qr-generator' || view === 'scanner' || view === 'global-scanner'
						? 'flex-start'
						: 'center'
				}
				alignItems={
			...
			>
				...
				{view === 'scanner' &&
					(mode === 'online' ? (
						<OnlineTicketModule isActive={!showPrompt} />
					) : (
						<OfflineTicketModule isActive={!showPrompt} />
					))}
				{view === 'global-scanner' && (
					<GlobalScannerModule
						isActive={!showPrompt}
			...
			{showPrompt && (
				...
				>
					{commandOutput && (
						...
						>
							<Text color={theme.textDim}>
								[<Clock format="full" />] ↳ {commandOutput}
							</Text>
						</Box>
					)}
					<Box>

...
const StepLine = ({
	...
	const getIdx = (s: string) =>
		['mode', 'printer', 'excel', 'loading', 'preview', 'scan'].indexOf(s);
			...
			{steps.map((s, i) => {
				...
				return (
					<Box key={s.id} alignItems="center">
						<Text
							color={
								isCurrent ? theme.primary : isPast ? theme.success : theme.text
							}
							bold={isCurrent}
							dimColor={!isCurrent && !isPast}
						>
							{isPast ? '●' : isCurrent ? '◎' : '○'} {s.label}
						</Text>
						{i < steps.length - 1 && (
							<Text color={theme.border} dimColor={true}>
								{' '}
								──{' '}
							</Text>
						)}
...
const LoadingMock: React.FC<{message: string}> = ({message}) => {
	...
	useEffect(() => {
		const t = setInterval(() => setFrame(f => (f + 1) % frames.length), 50);
	...
	return (
		...
		>
			...
			<Text color={theme.text}> {message.padEnd(30, ' ')}</Text>
		</Box>
	);
};
...
const openFileDialog = (): Promise<string | null> => {
	return new Promise(resolve => {
		...
		exec(
			`powershell -Sta -NoProfile -Command "${psScript}"`,
			(error, stdout) => {
				if (error) {
					resolve(null);
					return;
...
export const OfflineTicketModule: React.FC<OfflineTicketModuleProps> = ({
	isActive = true,
}) => {
	const {theme} = useTheme();
	...
	const [step, setStep] = useState<
		'mode' | 'printer' | 'excel' | 'loading' | 'preview' | 'scan'
	>('mode');
	...
	useEffect(() => {
		...
		setHistory(prev =>
			[{msg: scanMessage.text, type: scanMessage.type}, ...prev].slice(0, 10),
		);
	...
	const handleExecutePrint = (found: OfflineItem) => {
		if (
			...
			(lastProcessedSerial.current === found.serial && scanBuffer === '')
		)
			return;
		...
		task
			.then(() =>
				setScanMessage({text: `IMP: ${found.serial} OK`, type: 'success'}),
			)
			.catch(err =>
				setScanMessage({text: `ERR: ${err.message}`, type: 'error'}),
			)
			.finally(() => {
				isProcessing.current = false;
				setTimeout(() => {
					lastProcessedSerial.current = '';
				}, 1000);
			});
	};
	...
	useInput((input, key) => {
		...
		if (step === 'mode') {
			...
			if (key.upArrow)
				setPrinterSelectedIndex(p =>
					p <= 0 ? printerOptions.length - 1 : p - 1,
				);
			else if (key.downArrow)
				setPrinterSelectedIndex(p =>
					p >= printerOptions.length - 1 ? 0 : p + 1,
				);
			else if (key.return) {
				const s = printerOptions[printerSelectedIndex];
				if (s) {
					setSelectedPrinter(s.value === 'default' ? null : s.value);
					setStep('excel');
				}
			}
			...
			if (key.return) {
				...
				if (!f) {
					setScanMessage({
						text: `No existe la serie: ${v.toUpperCase()}`,
						type: 'error',
					});
					setScanBuffer('');
	...
	const handlePickFile = async () => {
		setExcelError('');
		setLoadingMsg('Abriendo explorador...');
		setStep('loading');
		const p = await openFileDialog();
		if (!p) {
			setExcelError('Selección cancelada.');
			setStep('excel');
			return;
		}
	...
	return (
		...
		>
			...
			>
				<Box>
					...
					<Text color={theme.text} bold wrap="truncate-end">
						{' '}
						ZENTRIA TICKET SYSTEM{' '}
					</Text>
				</Box>
				{columns > 80 && (
					<Text color={theme.text} bold dimColor={true}>
						v1.2.0 (ESTABLE)
					</Text>
				)}
			</Box>
			...
			<Box flexDirection={isCompact ? 'column' : 'row'} flexGrow={1}>
				...
				>
						...
						<Box flexDirection="column" justifyContent="center" flexGrow={1}>
							...
							<Box flexDirection="column" paddingLeft={2}>
								<Box
									borderStyle="double"
									borderColor={
										printMode === 'auto' ? theme.primary : theme.border
									}
									paddingX={1}
									marginBottom={1}
									width="100%"
								...
								<Box
									borderStyle="single"
									borderColor={
										printMode === 'manual' ? theme.primary : theme.border
									}
									paddingX={1}
									width="100%"
								>
						...
						<Box flexDirection="column" paddingX={1} flexGrow={1}>
							<Box marginBottom={1}>
								<Text bold color={theme.primary} wrap="truncate-end">
									Salida (Pag{' '}
									{Math.floor(printerSelectedIndex / visiblePrinters.length) +
										1}
									):
								</Text>
							</Box>
							<Box flexDirection="column" marginY={1}>
								{visiblePrinters.map(p => {
									const isS =
										printerOptions[printerSelectedIndex]?.value === p.value;
									const label = `${isS ? '➤ ' : '  '} ${p.label}`.padEnd(30);
									return (
										<Box
											key={p.value}
											paddingX={1}
											borderStyle={isS ? 'double' : undefined}
											borderColor={isS ? theme.primary : theme.border}
										>
											<SelectedGradient
												text={label}
												isActive={isS}
					...
					{(step === 'excel' || step === 'loading') && (
						...
						>
							{step === 'loading' ? (
								<LoadingMock message={loadingMsg} />
							) : (
								<>
									<Box marginBottom={1}>
						...
						>
							<Box flexDirection="column">
								<Text color={theme.success} bold>
									✓ Datos cargados ({items.length} registros)
								</Text>
								<Box
									flexDirection="column"
									marginY={1}
					...
					{step === 'scan' && (
						<Box flexDirection="column" flexGrow={1} justifyContent="center">
							...
							>
								...
								<Text color={theme.text} bold italic wrap="truncate-end">
									{scanBuffer || 'Esperando...'}
								</Text>
							</Box>
							{scanMessage && (
								<Box
				...
				>
					...
					>
						...
						<Box flexDirection="column" marginTop={1}>
							<Text color={theme.text} dimColor={true}>
								Modo:{' '}
					...
					<Box
						...
						borderColor={theme.border}
						paddingX={1}
						height={isCompact ? (isTall ? 10 : 6) : undefined}
					>
						<Box paddingX={1}>
							<Text color="black" backgroundColor={theme.border} bold>
								{' '}
						...
						<Box flexDirection="column" marginTop={1}>
							{history.length === 0 ? (
								<Text color={theme.text} dimColor={true} italic>
									Listo.
								</Text>
							) : (
			...
			<Box
				...
				width="100%"
			>
				<Box marginRight={isCompact ? 1 : 2} alignItems="center">
					<Kbd>ESC</Kbd>
					<Text color={theme.text} bold dimColor={true} wrap="truncate-end">
						{' '}
						Volver
					</Text>
				</Box>

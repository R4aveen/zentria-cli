...
console.log('\n✧ Exportando certificado...');
{
  ...
  const certScript = [
    ...
    `$certName = 'ZentriaCLI'`,
    `$canUseCertDrive = $true`,
    `if (-not (Get-PSDrive -Name Cert -ErrorAction SilentlyContinue)) {`,
    `  try { New-PSDrive -Name Cert -PSProvider Certificate -Root '\\' | Out-Null }`,
    `  catch { $canUseCertDrive = $false }`,
    `}`,
    `$cert = $null`,
    `if ($canUseCertDrive) {`,
    ...
    `  }`,
    `  if (-not $cert) {`,
    `    Write-Host '  Creando certificado autofirmado en almacén...'`,
    `    try {`,
    ...
    `} else {`,
    `  Write-Host '  Cert:\ no disponible, usando certificado temporal en memoria...'`,
    `  $rsa = [System.Security.Cryptography.RSA]::Create(2048)`,
    `  $req = New-Object System.Security.Cryptography.X509Certificates.CertificateRequest("CN=$certName", $rsa, [System.Security.Cryptography.HashAlgorithmName]::SHA256, [System.Security.Cryptography.RSASignaturePadding]::Pkcs1)`,
    `  $ku = New-Object System.Security.Cryptography.X509Certificates.X509KeyUsageExtension([System.Security.Cryptography.X509Certificates.X509KeyUsageFlags]::DigitalSignature, $false)`,
    ...
    `  $eku = New-Object System.Security.Cryptography.X509Certificates.X509EnhancedKeyUsageExtension($oids, $false)`,
    `  $req.CertificateExtensions.Add($ku)`,
    `  $req.CertificateExtensions.Add($eku)`,
    `  $tmpCert = $req.CreateSelfSigned((Get-Date).AddDays(-1), (Get-Date).AddYears(5))`,
    `  [System.IO.File]::WriteAllBytes('${CER_PATH}', $tmpCert.Export([System.Security.Cryptography.X509Certificates.X509ContentType]::Cert))`,
    `  Write-Host "  Thumbprint (temporal): $($tmpCert.Thumbprint)"`,
    `}`,
...
try {
  ...
  const signScript = [
    ...
    `    } catch {`,
    `      throw 'No se pudo inicializar el proveedor de certificados (Cert:).'`,
    `    }`,
    `  }`,
    `}`,
    `$stores = @('Cert:\\CurrentUser\\My', 'Cert:\\LocalMachine\\My')`,
